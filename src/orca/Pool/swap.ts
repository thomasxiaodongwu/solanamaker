...
import {swapQuoteByInputToken, IGNORE_CACHE, Whirlpool} from "@orca-so/whirlpools-sdk";
import {
...
export async function swap(
  ...
  if (side === "buy") {
    ...
    inToken = tokenMint;
    outToken = WSOL.mint;
    if (whirlPool.getTokenAInfo().mint.toBase58() === tokenMint.toBase58()) {
      tokenDecimal = whirlPool.getTokenAInfo().decimals;
    } else {
      tokenDecimal = whirlPool.getTokenAInfo().decimals;
    }
    quote = await swapQuoteByInputToken(
